...
class _NutritionAnalyticsWidgetState extends State<NutritionAnalyticsWidget> {
  ...
  Future<void> _loadNutritionData() async {
    ...
    try {
      ...
      final recentMealsFuture = NutritionService.instance.getUserMealsForDate(date: today);

      List<Future<Map<String, dynamic>>> dailySummaryFutures = [];
      for (int i = 0; i < widget.selectedPeriod; i++) {
        final date = today.subtract(Duration(days: i));
        dailySummaryFutures.add(NutritionService.instance.getDailyNutritionSummary(date: date));
      }
                  ...
                  bottomTitles: AxisTitles(
                    sideTitles: SideTitles(
                        ...
                        getTitlesWidget: (value, meta) {
                          ...
                          if (daysAgo == 0) return Text('Hoje', style: AppTheme.darkTheme.textTheme.bodySmall?.copyWith(color: AppTheme.textSecondary));
  ...
  Widget _buildRecentMeals() {
    final allFoodItems = _recentMeals.expand((meal) => (meal['meal_food_items'] as List? ?? [])).toList();
  ...
  Widget _buildMealItem(Map<String, dynamic> foodItem) {
    ...
    final name = food['name'] ?? 'Comida Desconhecida';
